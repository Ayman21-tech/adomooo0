...
import { 
  BookOpen, Eye, Brain, Lightbulb, BookA, 
...
const PREP_SELECTION_KEY = 'prep_selection';

interface PrepSelection {
  subjectId: string;
  chapterId: string;
}
...
export function PrepTab() {
  ...
  const prepButtons = [
    { id: 'notes', label: t('prep.notes', lang), icon: BookOpen, description: t('prep.notesDesc', lang), color: 'bg-primary/20 text-primary' },
    { id: 'revise', label: t('prep.revise', lang), icon: Eye, description: t('prep.reviseDesc', lang), color: 'bg-accent text-accent-foreground' },
    { id: 'visualize', label: t('prep.visualize', lang), icon: Brain, description: t('prep.visualizeDesc', lang), color: 'bg-secondary text-secondary-foreground' },
    { id: 'fix-confusion', label: t('prep.fixConfusion', lang), icon: Lightbulb, description: t('prep.fixConfusionDesc', lang), color: 'bg-muted text-muted-foreground' },
    { id: 'abbreviations', label: t('prep.abbreviations', lang), icon: BookA, description: t('prep.abbreviationsDesc', lang), color: 'bg-primary/10 text-primary' },
  ];
  
  const getInitialSelection = (): PrepSelection => {
    const urlSubject = searchParams.get('subject');
    const urlChapter = searchParams.get('chapter');
    if (urlSubject) return { subjectId: urlSubject, chapterId: urlChapter || '' };
    try {
      const stored = localStorage.getItem(PREP_SELECTION_KEY);
      if (stored) return JSON.parse(stored);
    } catch (e) { console.error('Error loading prep selection:', e); }
    return { subjectId: '', chapterId: '' };
  };
  
  const [selection, setSelection] = useState<PrepSelection>(getInitialSelection);
  ...
  useEffect(() => {
    if (selection.subjectId && hasBookPages) setShowHub(true);
  }, []);

  useEffect(() => {
    if (selection.subjectId) {
      localStorage.setItem(PREP_SELECTION_KEY, JSON.stringify(selection));
  ...
  const handleChapterChange = (chapterId: string) => {
    const resolvedId = chapterId === '__all__' ? '' : chapterId;
    setSelection(prev => ({ ...prev, chapterId: resolvedId }));
    setShowChapterSelect(false);
    if (pendingToolId) { navigateToTool(pendingToolId, chapterId); setPendingToolId(null); }
  };
  const handleStartPrep = () => { if (!hasBookPages) { navigate('/home?tab=syllabus'); return; } setShowHub(true); };
  const navigateToTool = (toolId: string, chapterId?: string) => {
    ...
    navigate(`/prep/${toolId}?subject=${selection.subjectId}${(chapterId || selection.chapterId) ? `&chapter=${chapterId || selection.chapterId}` : ''}`, { 
      state: { subjectId: selection.subjectId, chapterId: chapterId || selection.chapterId || undefined, chapterName: chapter?.chapter_name } 
    });
  };
  const handlePrepButtonClick = (buttonId: string) => {
    ...
    return (
      <div className="space-y-4 pb-20">
        <div className="text-center py-4">
          ...
          <h2 className="text-lg font-semibold mb-1">{t('prep.title', lang)}</h2>
          <p className="text-sm text-muted-foreground">{t('prep.subtitle', lang)}</p>
        </div>
        <Card className="glass-card border-border/50">
          <CardContent className="p-4 space-y-4">
            ...
            {selection.subjectId && !hasBookPages && (
              <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-xl flex flex-col items-center gap-3 text-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{t('prep.uploadFirstDesc', lang)}</p>
                </div>
            ...
            <Button className="w-full btn-premium text-primary-foreground" disabled={!selection.subjectId || !hasBookPages} onClick={handleStartPrep}>
              {t('prep.startPreparing', lang)}<ChevronRight className="w-4 h-4 ml-2" />
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  if (showChapterSelect && subjectChapters.length > 0) {
    const pendingTool = prepButtons.find(b => b.id === pendingToolId);
    return (
      <div className="space-y-4 pb-20">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">{lang === 'bangla' ? `${pendingTool?.label || 'এই টুলের'} জন্য অধ্যায় নির্বাচন করুন` : `Select a chapter for ${pendingTool?.label || 'this tool'}`}</p>
          </div>
          <Badge variant="outline" className="text-xs border-primary/30 text-primary">📖 {t('prep.title', lang)}</Badge>
        </div>
        <Card className="glass-card border-border/50">
          ...
          <CardContent className="space-y-2">
            {subjectChapters.map(ch => (
              <Button key={ch.id} variant="outline" className="w-full justify-start text-left h-auto py-3 hover:border-primary/50 hover:bg-primary/5" onClick={() => handleChapterChange(ch.id)}>
                <BookOpen className="w-4 h-4 mr-3 text-primary" />{ch.chapter_name}
              </Button>
            ))}
          </CardContent>
        </Card>
        <Button variant="ghost" className="w-full" onClick={handleGoBack}><ArrowLeft className="w-4 h-4 mr-2" />{t('prep.backToTools', lang)}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      {subjectChapters.length > 0 && (
        <Card className="glass-card border-border/50">
          <CardContent className="p-3">
            <Select value={selection.chapterId || '__all__'} onValueChange={handleChapterChange}>
      ...
      <Card className="glass-card border-border/50">
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2"><BookOpen className="w-4 h-4 text-primary" />{t('prep.studyTools', lang)}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-3">
            {prepButtons.map(btn => (
              <Button key={btn.id} variant="outline" className="h-auto py-4 px-4 flex flex-col items-center gap-2 text-center hover:border-primary/50 hover:bg-primary/5 transition-all" onClick={() => handlePrepButtonClick(btn.id)}>
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('prep.noChaptersDesc', lang)}</p>
